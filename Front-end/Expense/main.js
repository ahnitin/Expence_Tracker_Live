...
function AddUsertoScreen(obj)
{
        ...
        row.insertAdjacentHTML(
          "beforeend",
          `<td class='expense-name' >${obj.category}</td><td class='expense-date' >${obj.description}</td><td class='expense-Amount'>${obj.expenseamount}</td> <button class='delete-expense' id="deletebtn" >X </button>`
        );
...
expenseDatadiv.addEventListener("click", async(e) => {
  ...
  async function deleteme(id){
    const token = localStorage.getItem("token");
    await axios.delete("http://localhost:3002/deleteexpense/"+id,{headers: {"Authorization": token}})
}
...
function showLeaderBoard()
{
        ...
        LBbtn.onclick = async()=>{
            ...
            userLeaderBoard.data.forEach((element,i)=> {
                ...
                row.insertAdjacentHTML(
                  "beforeend",
                  `<td class='expense-name' >${i+1}</td><td class='expense-date' >${element.username}</td><td class='expense-Amount'>${element.total_cost}</td>`
                );
